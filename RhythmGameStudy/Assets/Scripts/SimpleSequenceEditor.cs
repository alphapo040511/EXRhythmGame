...
using UnityEngine;
using UnityEditor;                          //Editor 라이브러리 사용

public class SimpleSequenceEditor : EditorWindow
{
    private SequenceData sequenceData;          //시퀀스 데이터 저장 객체
    private Vector2 scrollPos;                  //스크롤 뷰 위치
    private float beatHeight = 20;              //각 비트의 높이
    ...
    {
        EditorApplication.update += Update;             //업데이트 함수를 이벤트로 등록 (에디터 안에서 업데이트를 만들어 줌)
        CreateAudioSource();                            //오디오를 사용할 것이기 때문에 오디오 등록을 해준다.
    }

    private void OnDisable()                            //윈도우 에디터 창을 닫았을 때
    {
        EditorApplication.update -= Update;             //업데이트 함수를 이벤트 해제 (안 그러면 업데이트가 쌓이며 문제가 생김)
    ...
    private void InitializeTracks()                     //시퀀스 데이터가 들어왔을 때 데이터를 셋팅 해주는 함수
    {
    ...
    private void Update()
    {
        ...
        {
            EditorApplication.update -= Update;     //이벤트 할당을 해제 해준다
            return;
        }
        ...
        if(isPlaying && audioSource != null && audioSource.isPlaying)
        {
            ...
            currentBeatTime = Mathf.FloorToInt(elapseTime * sequenceData.bpm / 60f);        //BPM 분의 60에 흐른 시간을 곱하면 현제 비트이다.

            if(currentBeatTime >= totalBeats)
            {
                StopPlayBack();
            }
            Repaint();                          //업데이트가 끝난후에 리페인트 (화면 갱신을 해주지 않는 업데이트 이기 때문에)
        }
    }

    private void StartPlayBack(int fromBeat)                        //오디오 특정 비트부터 플레이 해주는 함수
    {
        if (sequenceData == null || sequenceData.audioClip == null || audioSource == null) return;       //3가지 중 나라도 없으면 리턴
        ...
        {
            audioSource.clip = sequenceData.audioClip;              //지금 시퀀스 데이터를 클립으로 변경 시켜준다.
        }
        ...
        this.startTime = (float)EditorApplication.timeSinceStartup - startTime;     //에디터 시간을 반영한다.
    ...
    private void DrawBeat(int trackIndex, int beatIndex)
    {
        ...
        //현재 트랙의 현재 비트 인덱스에 해당하는 노트 값을 가져오면서 검사해서 유효한 인데스가 아닐경우 0으로 설정된다.
    ...
    private void OnGUI()
    {
        ...
        {
            EditorGUILayout.EndVertical();              //BeginVertical()와 쌍
            return;
        }
        ...
        int newNumberOfTrack = EditorGUILayout.IntField("트랙 수", sequenceData.numberOfTracks);
        if(newNumberOfTrack != sequenceData.numberOfTracks)                 //트랙 수가 다를 경우
        {
            sequenceData.numberOfTracks = newNumberOfTrack;                 //트랙 수를 가져와서 갱신
            InitializeTracks();
        }
        ...
        {
            totalBeats = Mathf.FloorToInt((sequenceData.audioClip.length / 60f) * sequenceData.bpm);        //비트수를 계산한다.
        }
        else
        {
            totalBeats = 0;
        ...
        EditorGUILayout.BeginHorizontal();                      //가로 UI 설정 시작

        if (GUILayout.Button(isPlaying ? "일시정지" : "재생"))
        {
            if (isPlaying) PausePlayBack();                     //재생중이면 멈춤
            else StartPlayBack(currentBeatTime);                //일시정지면 다시 재생
        }

        if(GUILayout.Button("처음부터 재생"))
        {
            StartPlayBack(0);                                   //0번째 비트부터 재생
        }
        EditorGUILayout.EndHorizontal();                        //가로 UI 설정 종료
        ...
        for(int j = 0; j < totalBeats; j++)
        {
            ...
            GUILayout.Space(beatHeight - EditorGUIUtility.singleLineHeight);
        }
        EditorGUILayout.EndVertical();
